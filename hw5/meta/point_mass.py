...
class PointEnv(Env):
    ...
    def reset_task(self, is_evaluation=False, grain_size=None):
        ...
        if (not is_evaluation):
            ...
            y0 = np.random.choice(np.arange(-10 + grain_size * (x0 // grain_size % 2), 10, grain_size * 2)) # CHECKER: grain_size * 2
        else:
            x0 = np.random.choice(np.arange(-10, 10, grain_size))
            y0 = np.random.choice(np.arange(-10 + grain_size * ((x0 // grain_size + 1) % 2), 10, grain_size * 2)) # CHECKER: grain_size * 2
