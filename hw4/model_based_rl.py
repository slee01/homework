...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        for epoch in range(self._training_epochs):
            for state, action, next_state, _, _ in dataset.random_iterator(self._training_batch_size):
                loss = self._policy.train_step(states=state, actions=action, next_states=next_state)
                losses.append(loss)
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            for t in range(states.shape[0]):
                pred_state = self._policy.predict(state, actions[t])
